...
public final class GridRuntime
{
	...
	static GridMasterManager masterManager = new GridMasterManager();
	...
	public static boolean initialize(GridConfiguration config)
	{
		...
		RpcExecutor.registerSystemMethod();
		...
		ret = masterManager.initialize(config);
        if (!ret)
        {
            log.error("Fail to initialize master manager.");
            return false;
        }
